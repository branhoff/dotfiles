...
import { pathExists, readFile, readdir, stat, ensureDir, writeFile, ensureDirSync } from 'fs-extra';
import { join } from 'path';
import simpleGit from 'simple-git';
import { randomUUID } from 'crypto';

const getUserReposPath = (baseDir, username) => join(baseDir, username);
const getRepoPath = (baseDir, username, recipeId) => join(baseDir, username, recipeId);
const getRecipeFilePath = (baseDir, username, recipeId) => join(baseDir, username, recipeId, 'recipe.md');
...
const getRepoMetadata = async (repoPath, recipeId) => {
  try {
    ...
    const recipeFile = join(repoPath, 'recipe.md');
...
const getAllRecipes = async (req, res, { baseDir }) => {
  try {
    ...
    const recipeDirs = await readdir(userPath);
    const recipes = [];
    ...
    for (const recipeId of recipeDirs) {
      ...
      if (!(await stat(repoPath)).isDirectory()) continue;
...
const setupApp = (config = {}) => {
  ...
  const baseDir = config.reposDir || join(__dirname, 'git-repos');
